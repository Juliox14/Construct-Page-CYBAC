...
const Filter = ({ open, handleFilterUpdate }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const getData = await handleFilter(
                data.estado,
                data.precioInicial,
                data.precioFinal
            );
    ...
    return (
        <motion.div
            variants={{
                visible: { opacity: 0, y: 0, width: '100%', height: 0 },
                hidden: { y: 20, opacity: 1, width: '100%', height: 240 },
            }}
            animate={open ? 'visible' : 'hidden'}
            className={classes.filtro}
        >
            <div className={classes.boxFilterList}>
                <h2 className={classes.filtro_boxFilterList__title}>
                    Lista de filtros
                </h2>
                    ...
                    {filtros &&
                        filtros.map((filtro, index) => (
                            <Button
                                ...
                                onClick={(e) => {
                                    handleDeleteFilter(index, filtro);
                                }}
                            >
                                {filtro}
                            </Button>
                ...
                <div className={classes.filtro_boxBtnFilter} margin="normal">
                    <div className="cajaSlider">
                        <div className={classes.filtro_wrapper}>
                            <div
                                className={classes.filtro_priceInput}
                                ref={priceInput}
                            >
                                ...
                                <div
                                    className={
                                        classes.filtro_priceInput__separator
                                    }
                                >
                                    -
                                </div>
                                ...
                                {alertMinVal && (
                                    <span className={classes.filtro_alert}>
                                        El valor minimo es mayor o igual al
                                        maximo
                                    </span>
                                )}
                            </div>
                            <div className={classes.filtro_slider}>
                                <div
                                    className={`${classes.filtro_slider__progress}`}
                                    ref={progress}
                                ></div>
                            </div>
                            <div
                                className={classes.filtro_rangeInput}
                                ref={sliderReference}
                            >
                                <input
                                    type="range"
                                    className={`rangeMin ${classes.filtro_inputTypeRange}`}
                                    id={classes.filtro_inputTypeRange}
                                    min="0"
                                    max="5000000"
                                    defaultValue="0"
                                    step="1000"
                                ...
                                <input
                                    type="range"
                                    className={`rangeMax ${classes.filtro_inputTypeRange}`}
                                    id={classes.filtro_inputTypeRange}
                                    min="0"
                                    max="5000000"
                                    defaultValue="1250000"
            ...
            <div className={classes.filtro_boxLoading}>
                <Button
                    variant="contained"
                    className={classes.filtro_boxLoading__btnLoading}
