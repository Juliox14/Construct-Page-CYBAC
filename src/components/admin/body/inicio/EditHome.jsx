...
const EditHome = ({hero, srcImages}) => {
    ...
    [confirmationUpdateDelete, setConfirmationUpdateDelete] = useState([false, false]),
    ...
    const handleDelete = async(e) => {
        ...
        if(!compareObjects(clientData, hero) && compareObjects([...newDataSlider], newData)){
    ...
    const handleSubmit = async (e) => {
        ...
        const vida = responseImageUrl.data.url.filter(item => dataSlider.some((url) => url.id === item.id));
        ...
        const muerte = responseImageUrl.data.url.filter(item => newDataSlider.some((url) => url.id === item.id));
        ...
        if(responseImageUrl.status === 200){
            const response = await axios.put("/api/home", [tempDataSlider, tempNewDataSlider]);
                    ...
                    <form className={classes.homeEdit_formSubservicio} ref={refForm} onSubmit={(e) => {handleSubmit(e)}}>
                        ...
                        style={{boxShadow: theme.palette.mode === "dark" ? "0px 0px 0px 0px white" : "0px 0px 20px 0px #ccc"}}>
                            ...
                            <IconButton sx={{
                                    ...
                                    top: '5%',
                                    right: '2%',
                                }} onClick={()=> handleShowDeleteConfirmation(heroItem.id)}>
                                        ...
                                        init={{
                                            ...
                                            setup: (editor) => {
                                                editor.on('NodeChange', (e) => {
                                                  ...
                                                  if (editor.getBody().firstChild && editor.getBody().firstChild.nodeName === 'P') {
                        ...
                        <div className={classes.homeEdit_formSubservicio_updateAlert}>
                            ...
                            <Button variant="contained" color="primary" onClick={addNewData}
                                sx={{
                                    width: "200px",
                                    height: "50px",
                                }} disabled={disableButton}> Agregar un elemento </Button>
                        </div>
                    </form>
                </Box>
        </div>
