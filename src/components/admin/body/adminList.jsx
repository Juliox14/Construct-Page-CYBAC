...
export default function AdminList({ nombre_elemento, descripcion, link, id, url, video, delete_button }) {
    ...
    const handleDelete = async () => {
        ...
        if (response.status === 200) {
            ...
            alert("Error al eliminar el elemento, comuniquese con CYBAC TI");
        }
    }
            ...
            {delete_button === false || delete_button === undefined ? (
                ...
                <Box component="section" className={classes.adminList_boxFatherItemList} sx={{
                    ...
                    <div onMouseOver={handleOver} onMouseOut={handleOut}>
                        {hover && (<div className={classes.adminServiceList_boxFatherItemList_editBox} style={{
                            backgroundColor: theme.palette.mode === 'dark' ? "#2f2f2fb4" : "#ffffffa4", display: "flex", gap: '20px'
                        }}>
                            <Link href={link}><EditIcon fontSize="large" sx={{ color: theme.palette.mode === 'dark' ? "#ADA479" : "#014655" }} /></Link>
                            <button onClick={() => setDeleteConfirm(true)} style={{ border: 'none', background: 'none', padding: '0', cursor: 'pointer' }}><DeleteIcon fontSize="large" sx={{ color: '#A50505' }} /></button>
                        </div>)}
