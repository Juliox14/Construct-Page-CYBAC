    ...
    switch (req.method) {
        case 'PUT':
            try {
                ...
                for (let i = 0; i < redesSocialesData.length; i++) {
                    parseRedesSocialesData.push(
                        parseData(redesSocialesData[i])
                    );
                }

                for (let i = 0; i < parseRedesSocialesData.length; i++) {
                    await db.query(
                        `UPDATE redes_sociales SET ${parseRedesSocialesData[i]} WHERE id_red_social = ${redesSocialesData[i].id_red_social}`
                    );
                }
                res.status(200).json({
        ...
        case 'POST':
            try {
                ...
                for (let i = 0; i < redesSocialesData.length; i++) {
                    await db.execute(
                        `INSERT INTO redes_sociales (red_social, enlace_red_social, icono_red_social) VALUES(? , ?, ?)`,
                        [
                            redesSocialesData[i].red_social,
                            redesSocialesData[i].enlace_red_social,
                            redesSocialesData[i].icono_red_social,
                        ]
                    );
                }
                res.status(200).json({
