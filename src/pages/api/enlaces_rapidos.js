        ...
        case 'PUT':
            try {
                ...
                for (let i = 0; i < enlacesRapidosData.length; i++) {
                    parseEnlacesRapidosData.push(parseData(enlacesRapidosData[i]));
                }

                for (let i = 0; i < parseEnlacesRapidosData.length; i++) {
                    await db.query(`UPDATE enlaces_rapidos SET ${parseEnlacesRapidosData[i]} WHERE id_enlace = ${enlacesRapidosData[i].id_enlace}`);
                }
                res.status(200).json({ message: 'Enlace rápido actualizado correctamente' });
            } catch (error) {
                console.error(error);
