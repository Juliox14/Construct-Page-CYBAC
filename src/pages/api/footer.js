    ...
    switch (req.method) {
        case 'GET':
            try {
                ...
                const [enlaces_rapidos] = await db.query(
                    'SELECT * FROM enlaces_rapidos'
                );
                const [redes_sociales] = await db.query(
        ...
        case 'PUT':
            try {
                ...
                await db.query(
                    `UPDATE footer SET ${parseFooter} WHERE id_footer = ${footer.id_footer}`
                );
                return res
                    .status(200)
                    .json({ message: 'Footer actualizado correctamente' });
            } catch (error) {
                return res
                    .status(500)
                    .json({ message: 'Error en el servidor' });
