...
//Imports de estilos.
import classes from "../../../../components/admin/body/nosotros/EditService.module.scss";

//Imports de librerias externas.
import axios from "axios";

const breadcrumbClients = ({dataSSR}) => {
    console.log(dataSSR);
    ...
    const handleSubmit = async (e) => {
        ...
        if(responseImageUrl.status === 200){
            const response = await axios.put(`/api/home_clients`, updatedDataWithImg);
        ...
        <>
            ...
            <Box sx={{
                    ...
                    <form className={classes.homeEdit_formSubservicio} onSubmit={handleSubmit} ref={refForm}>
                        <div>
                            <div>
                                <label htmlFor={`subtitulo_about_${data.subTitle_breadcrumb}`}>Subtitulo</label>
                                <textarea name="subtitulo_about" id={`subtitulo_about_${data.subTitle_breadcrumb}`} rows={10} cols={40} style={{resize: "none", backgroundColor: theme.palette.mode === 'dark' ? "#222F3E" : "white", color: theme.palette.mode === 'dark' ? "white" : "black", transition: `background-color ${theme.transitions.duration.standard}ms`}}
                                onChange={(e) => {
                                    const updatedData = data;
                                    updatedData.subTitle_breadcrumb = e.target.value;
                                    setData(updatedData);
                                }}>{data.subTitle_breadcrumb}</textarea>
                            </div>
                            <div>
                                <label htmlFor={`title_about_${data.title_breadcrumb}`}>Título</label>
                                <textarea name="title_about" id={`title_about_${data.title_breadcrumb}`} rows={10} cols={40} style={{resize: "none", backgroundColor: theme.palette.mode === 'dark' ? "#222F3E" : "white", color: theme.palette.mode === 'dark' ? "white" : "black", transition: `background-color ${theme.transitions.duration.standard}ms`}}
                                onChange={(e) => {
                                    const updatedData = data;
                                    updatedData.title_breadcrumb = e.target.value;
                                    setData(updatedData);
                                }}>{data.title_breadcrumb}</textarea>
                            </div>
                            <div>
...
export async function getServerSideProps() {
    const aboutItems = await getElement('home_clients');
    return {
        props: {
            dataSSR: aboutItems[1][0],
        },
    };
}